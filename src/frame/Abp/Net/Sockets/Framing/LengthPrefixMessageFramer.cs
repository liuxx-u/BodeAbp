        ...
        private void Parse(ArraySegment<byte> bytes)
        {
            ...
            for (int i = bytes.Offset, n = bytes.Offset + bytes.Count; i < n; i++)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        System.Buffer.BlockCopy(bytes.Array, i, _messageBuffer, _bufferIndex, copyCnt);
                    }
                    catch (Exception ex)
                    {
                        Logger.Error(string.Format("Parse message buffer failed, _headerLength: {0}, _packageLength: {1}, _bufferIndex: {2}, copyCnt: {3}, _messageBuffer is null: {4}",
                            _headerBytes,
                            _packageLength,
                            _bufferIndex,
                            copyCnt,
