...
namespace BodeAbp.Queue.Broker
{
    ...
    public class Queue : IQueue
    {
        ...
        public Queue(string topic, int queueId)
        {
            ...
            _jsonSerializer = IocManager.Instance.Resolve<ISerializer<string>>();
            ...
            _logger = IocManager.Instance.Resolve<ILoggerFactory>().Create(this.GetType().FullName);
        }
        ...
        private QueueSetting LoadQueueSetting()
        {
            ...
            using (var stream = new FileStream(_queueSettingFile, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
            {
                using (var reader = new StreamReader(stream))
                {
                    ...
                    {
                        return _jsonSerializer.Deserialize<string,QueueSetting>(text);
                    }
                    return null;
                }
            }
