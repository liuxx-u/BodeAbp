...
namespace Abp.Rpc.Client.HealthChecks
{
    public class DefaultHealthCheckService : IHealthCheckService, IDisposable
    {
        ...
        public Task Monitor(AddressBase address)
        {
        ...
        public Task<bool> IsHealth(AddressBase address)
        {
            return Task.Run(() =>
            {
        ...
        /// <param name="address">地址模型。</param>
        /// <returns>一个任务。</returns>
        public Task MarkFailure(AddressBase address)
        {
            return Task.Run(() =>
            {
                var key = address.ToString();
        ...
        private void Remove(IEnumerable<AddressBase> addressModels)
        {
        ...
        {
            public MonitorEntry(AddressBase addressModel, bool health = true)
            {
