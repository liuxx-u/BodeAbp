...
namespace Abp.Helper
{
    public static class StringHelper
    {
        ...
        public static string GetChineseSpell(string chStr)
        {
            //验证是否输入汉字
            ...
            foreach (char c in mChar)
            {
                //如果输入的是汉字
                if (reg.IsMatch(c.ToString()))
                {
                    var arr = System.Text.Encoding.Default.GetBytes(c.ToString());
                    int m1 = (short)(arr[0]);
                    int m2 = (short)(arr[1]);
        ...
        //定义拼音区编码数组
        private static int[] _getValue = new int[]
            {
        ...
        //定义拼音数组
        private static string[] _getName = new string[]
            {
